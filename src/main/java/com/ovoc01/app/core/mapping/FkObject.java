...
public class FkObject {
    ...
    public <T> T init(Connection c) throws Exception {
        DtbObjectAccess dtbObjectAccess = (DtbObjectAccess) getViewClass().newInstance();
        Field primaryKey = dtbObjectAccess.getPrimaryKey();
        if (primaryKey == null) {
            throw new IllegalArgumentException("Your object's primary key is null");
        }
        Method setter = dtbObjectAccess.getClass().getDeclaredMethod(Utils.createSetter(primaryKey.getName()), primaryKey.getType());
        setter.invoke(dtbObjectAccess, fkValue);

        T[] selecTs = dtbObjectAccess.select(c);
